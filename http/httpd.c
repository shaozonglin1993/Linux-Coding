...
static void exe_cgi(int sock, const char* method, const char *path, const char *query_string)
{
	...
	if(strcasecmp(method, "GET") == 0){
		...
		do{
			...
			if(strcasecmp(buf, "Content-Length: ", 16) == 0){
				conetent_length = atoi(&buf[16]);
				//break;
			}
		}while((ret > 0) && (strcmp(buf,"\n") != 0));
		if(content_length == -1){
...
static int startup(const char* _ip, int _prot)
{
	...
	int opt = 1;
	setsocketopt(sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

	struct sockaddr_in local;
	local.sin_family = AF_INET;
	local.sin_port = htons(_port);
	loacl.sin_addr.s_addr = inet_addr(_ip);

	if(bind(sock, (struct sockaddr*)&Local, sizeoff(local)) < 0){
		perror("bind");
		exit(3);
	}
